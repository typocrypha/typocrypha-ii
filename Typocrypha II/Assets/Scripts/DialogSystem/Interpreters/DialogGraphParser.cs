...
public class DialogGraphParser : GraphParser
{
    ...
    private readonly Stack<BaseNode> recStack = new Stack<BaseNode>();
    private readonly Queue<DialogNode> skipQueue = new Queue<DialogNode>();
    ...
    public DialogItem NextDialog(bool next)
    {
        ...
        if (currNode is DialogNode dialogNode)
        {
            GetDialogData(dialogNode, out var charNames, out var cds, out var voice, out string displayName);
            // Add to history.
            DialogHistory.instance.AddHistory(displayName, dialogNode.text);
            if (skipQueue.Count > 0 && dialogNode == skipQueue.Peek())
            {
                skipQueue.Dequeue();
                return new DialogItemQueued();
            }
            else if (currNode is DialogNodeVN vnNode)
            {
            ...
            {
                var anItem = CreateDialogItemAN(anNode, voice);
                if (anNode.layoutSetting == TextAnchor.MiddleLeft)
                {
                    foreach (var node in LookAhead())
                    {
                        if (node is ClearNode)
                            break;
                        if (!(node is DialogNode))
                            continue;
                        if (!(node is DialogNodeAN futureAnNode))
                            break;
                        if (futureAnNode.layoutSetting != TextAnchor.MiddleLeft)
                            break;
                        GetDialogData(dialogNode, out _, out _, out var futureV, out _);
                        anItem.QueuedItems.Add(CreateDialogItemAN(futureAnNode, futureV));
                        skipQueue.Enqueue(futureAnNode);
                    }
                }
                return anItem;
            }
    ...
    private DialogItemAN CreateDialogItemAN(DialogNodeAN anNode, List<AudioClip> voice)
    {
        return new DialogItemAN(anNode.text, voice, anNode.alignmentOptions, anNode.layoutSetting);
    }

    private void GetDialogData(DialogNode dialogNode, out string[] charNames, out List<CharacterData> cds, out List<AudioClip> voice, out string displayName)
    {
        charNames = dialogNode.characterName.Split(new char[] { '|' }, new char[] { '\\' });
        cds = new List<CharacterData>(charNames.Length);
        voice = new List<AudioClip>(charNames.Length);
        ...
        displayName = (dialogNode.displayName.Trim().Length == 0)
                           ? dialogNode.characterName
                           : dialogNode.displayName;
    }
