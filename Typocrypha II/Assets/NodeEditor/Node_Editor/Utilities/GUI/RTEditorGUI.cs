...
namespace NodeEditorFramework.Utilities 
{
	public static class RTEditorGUI 
	{
        ...
        public static float lineHeight { get { return GUI.skin.label.CalcHeight(new GUIContent("i"), labelWidth); } }
		...
		{
			return ObjectField<T> (GUIContent.none, obj, allowSceneObjects, options);
		}
		...
		{
			return ObjectField<T> (new GUIContent (label), obj, allowSceneObjects, options);
		}
		...
		public static T ObjectField<T> (GUIContent label, T obj, bool allowSceneObjects, params GUILayoutOption[] options) where T : Object
		{
			#if UNITY_EDITOR
			if (!Application.isPlaying)
				return UnityEditor.EditorGUILayout.ObjectField (label, obj, typeof (T), allowSceneObjects, options) as T;
			#endif
			bool open = false;
		...
		{
			return EnumPopup (new GUIContent (label), selected, options);
		}
